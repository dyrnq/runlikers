...
docker network rm runlike_fixture_bridge 2>/dev/null || true
docker network create runlike_fixture_bridge

docker network rm runlike_custom-net 2>/dev/null || true
docker network create --subnet=10.10.0.0/16 runlike_custom-net
...
docker network rm runlike_fixture_ipv6 2>/dev/null || true
docker network create --ipv6 --subnet=2001:db8::/64 runlike_fixture_ipv6

docker run -d --name runlike_fixture8 \
    --network runlike_fixture_ipv6 \
    --ip6 2001:db8::42 \
    runlike_fixture \
    bash -c 'bash sleep.sh'

docker run -d --name runlike_fixture7 \
    --rm \
    --entrypoint /bin/bash \
    runlike_fixture \
    sleep.sh
