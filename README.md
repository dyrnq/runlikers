...
Ever looked at a running container and wondered _"how did I start this thing?"_  
`runlikers` inspects a container and prints the exact `docker run` command needed to reproduce it — ports, volumes, env vars, labels, resource limits, and everything else.

Inspired by [runlike](https://github.com/lavie/runlike) (Python), rewritten in Rust for single-binary distribution and better performance.

## Features

- **Two data backends**: bollard API (default, with image-aware filtering) or `docker inspect` CLI (`--inspect`)
- **55+ supported options**: from `--volume` and `-p` to `--ulimit`, `--health-cmd`, `--sysctl`, and more
- **Pretty mode** (`-p`): multi-line output with `\` line continuations
- **Tidy mode** (`--tidy`): filter out Docker daemon defaults (e.g. `--runtime=runc`, default `--shm-size`, `--ipc=private`)
...
| `--tidy`          | Filter out Docker daemon defaults (runtime, shm-size, ipc, stop-signal) |
...
| `-v, --volume`                              | `HostConfig.Mounts` / `HostConfig.Binds`                      |
| `--mount`                                   | `HostConfig.Mounts` / `HostConfig.Binds` (via `--mount` flag) |
| `--tmpfs`                                   | `HostConfig.Mounts` (type=tmpfs)                              |
...
| `--cpuset-cpus`                             | `HostConfig.CpusetCpus`                                       |
| `--cpuset-mems`                             | `HostConfig.CpusetMems`                                       |
...
| `--device-read-bps`                         | `HostConfig.BlkioDeviceReadBps`                               |
| `--device-read-iops`                        | `HostConfig.BlkioDeviceReadIOps`                              |
| `--device-write-bps`                        | `HostConfig.BlkioDeviceWriteBps`                              |
| `--device-write-iops`                       | `HostConfig.BlkioDeviceWriteIOps`                             |
...
`runlikers` supports two ways to read container data:

### bollard API (default)

Uses the [bollard](https://github.com/fussybeaver/bollard) Rust library to talk to the Docker daemon via its REST API.  
This path also fetches image metadata, allowing it to filter out options inherited from the image (e.g., `ENV` lines from the Dockerfile, image `LABEL`s, default `ENTRYPOINT`).
...
Read raw `docker inspect` JSON from stdin — useful for scripting or offline analysis.
...
Filters out options that are likely Docker daemon defaults rather than user-specified:

- `--runtime=runc` → default runtime
- `--ipc=private` / `--ipc=shareable` → default IPC mode
- `--shm-size="67108864"` → default 64 MB shared memory
- `--stop-signal=<value>` → only filtered when it matches the image's `StopSignal`

## Example

```bash
# Inspect an nginx container
$ runlikers -p nginx
...
# With --tidy, defaults are filtered out
...
Both bollard (default) and `--inspect` modes work inside the container.
